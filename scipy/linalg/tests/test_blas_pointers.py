...
class test_wfunc_pointers(TestCase):
    ...
    def test_double_args(self):
        ...
        assert_allclose(blas.dasum(x), 8.5, 10)
        assert_allclose(blas.ddot(x, y), 6.75, 10)
        assert_allclose(blas.dnrm2(x), 5.85234975815, 10)

        assert_allclose(blas.dasum(x[::2]), 5.5, 10)
    ...
    def test_double_complex_args(self):
        ...
        assert_allclose(blas.zdotc(cx[::2], cy[::2]), -18.5625+22.125j, 10)
        assert_allclose(blas.zdotu(cx[::2], cy[::2]), -6.5625+31.875j, 10)
