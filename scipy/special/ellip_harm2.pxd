...
cdef inline double ellip_harmonic( double coord_a, double coord_b, double coord_c, int n,int p, double l, double signn, double signm) nogil:
    ...
    if coord_a < coord_b:
        sf_error.error("ellip_harm",sf_error.ARG,"coord_a should be greater than coord_b")
        return nan

    if coord_b < coord_c:
        sf_error.error("ellip_harm",sf_error.ARG,"coord_b should be greater than coord_c")
        return nan
    ...
    for i in range(0,size):
        s[i] = 1 / s[i]
#    if coord_a >0:
#        return eigv[(size-1)*size + size-1]
