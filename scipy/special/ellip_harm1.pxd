...
cdef inline double ellip_harmonic( double coord_a, double coord_b, double coord_c, int n,int p, double l, double signn, double signm) nogil:
    ...
    cdef int r, tp, j, size, i, info, lwork
    cdef char t
    ...
    cdef double *wr = <double *> malloc(sizeof(double) * size)
    cdef double *wi = <double *> malloc(sizeof(double) * size)
    ...
    if coord_b < 0:
        return m[size-1]
    c_dgeev("N", "V", &size, <double *> m, &size,<double *> wr,<double *> wi, NULL, &size,<double *> eigv, &size, work, &lwork, &info)              	 
    if info != 0: 
    ...
    for i in range(0,size *size):
        m[size*size -1-i] = eigv[i]
        if(<int> i%size == <int> i/size):
              m[size*size-1-i] = -1 * m[size*size-1-i]
    
 #   for i in range(0,size):
  #      for j in range(0,size):
   #           eigv[i*size+j] = m[j*size+i]
    d = m + tp*size
